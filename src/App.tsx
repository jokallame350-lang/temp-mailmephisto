...
const App: React.FC<AppProps> = ({ hideSEOContent = false, hideFooter = false, hideHeroBanner = false, hideNavbar = false }) => {
  ...
  const { emails, selectedEmailId, currentEmailDetail, isLoadingDetail, isLoadingEmails, fetchError, progress, searchQuery, stats, notifFilters, unreadMap, setSearchQuery, setSelectedEmailId, setNotifFilters, handleManualRefresh, handleDeleteEmail, handleDeleteAllEmails, incrementAccountStat } = useEmails(activeAccount, accounts, addToast, autoVerifyEnabled, handleAutoVerifySuccess);
