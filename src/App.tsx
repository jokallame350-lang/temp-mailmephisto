...
const App: React.FC<AppProps> = ({
  ...
  const addToast = useCallback((from: string, subject: string) => {
    ...
    setToasts(prev => [{ id: uniqueId, from, subject, code: code || undefined }, ...prev.filter(t => t.subject !== subject || t.from !== from)].slice(0, 3));
  }, []);
  ...
  const toggleAutoVerify = useCallback(() => {
    setAutoVerifyEnabled(prev => {
      ...
      addToast(next ? '⚡ Otomatik Doğrulama Aktif' : 'Otomatik Doğrulama Kapalı', next ? 'Gelen üyelik onay linkleri arka planda otomatik tıklanacak.' : 'Otomatik tıklama modu kapatıldı.');
      return next;
    });
  }, [addToast]);

  const handleAutoVerifySuccess = useCallback((urlLabel: string) => {
    addToast('⚡ Otomatik Doğrulandı!', `"${urlLabel}" aktivasyon bağlantısı başarıyla tetiklendi.`);
  }, [addToast]);
  ...
  } = useEmails(activeAccount, accounts, addToast, autoVerifyEnabled, handleAutoVerifySuccess);
  ...
  const handleDeleteActiveAccount = useCallback(() => {
    if (activeAccount) deleteAccount(activeAccount.id);
  ...
  useEffect(() => {
    ...
    document.title = unreadCount > 0 && document.hidden ? `(${unreadCount}) ${baseTitle}` : baseTitle;
    const handleFocus = () => {
  ...
  const handleCreateCustom = useCallback(async (username: string, domain: string, apiBase: string) => {
    ...
    if (result.success) {
      ...
      const title = isTaken ? (lang === 'tr' ? 'Kullanıcı Adı Kullanımda' : 'Username Taken') : (lang === 'tr' ? 'Hesap Oluşturulamadı' : 'Creation Failed');
      const msg = isTaken ? (lang === 'tr' ? `"${username}@${domain}" adresi zaten başkası tarafından alınmış. Lütfen farklı bir kullanıcı adı veya domain seçin.` : `"${username}@${domain}" is already taken. Please choose a different username.`) : t.connError;
  ...
  const handleNewAccount = useCallback(async () => {
    ...
    if (result.success) incrementAccountStat();
  ...
  const handleDomainChange = useCallback(async (newDomain: string) => {
    ...
    if (res.success && res.address) {
      addToast(lang === 'tr' ? '🌐 Domain Değiştirildi' : '🌐 Domain Switched', lang === 'tr' ? `Yeni adres: ${res.address}` : `New address: ${res.address}`);
    }
  ...
  return (
    <ErrorBoundary>
      <div className={hideHeroBanner ? "w-full flex flex-col font-['Sora'] text-slate-200 relative overflow-x-hidden" : "min-h-screen flex flex-col font-['Sora'] bg-[#050505] text-slate-200 overflow-x-hidden relative"}>
        <SEOHead lang={lang} />
        <SkipNavigation />
          ...
          {!hideHeroBanner && (
            <div className="w-full max-w-4xl flex flex-col items-center text-center space-y-4 relative">
              <div className="space-y-2">
                ...
                <p className="text-sm md:text-base text-slate-400 max-w-lg mx-auto leading-relaxed">{t.heroTagline}</p>
              </div>
              <div className="flex items-center gap-4 text-[10px] text-slate-400 font-medium uppercase tracking-widest">
                <span className="flex items-center gap-1"><ShieldCheck className="w-3 h-3 text-orange-500/60" aria-hidden="true" />{t.heroBadge1}</span>
