...
export const onTokenRefresh = (cb: TokenRefreshCallback) => { tokenRefreshListener = cb; return () => { if (tokenRefreshListener === cb) tokenRefreshListener = null; }; };
...
const randomInt = (max: number): number => {
  ...
  // Passwords must never fall back to Math.random(). This branch is only for non-secret mailbox naming.
  return Date.now() % max;
};
...
const formatSenderName = (fromAddress: string): string => {
  ...
  const brands: Record<string, string> = { instagram: 'Instagram', cloudflare: 'Cloudflare', google: 'Google', netflix: 'Netflix', facebook: 'Facebook', twitter: 'X (Twitter)', 'x.com': 'X (Twitter)', github: 'GitHub', spotify: 'Spotify', discord: 'Discord', telegram: 'Telegram', steam: 'Steam', epicgames: 'Epic Games', microsoft: 'Microsoft', apple: 'Apple' };
  for (const [key, value] of Object.entries(brands)) if (lower.includes(key)) return value;
  const [userPart = '', domainPart = ''] = fromAddress.split('@');
  if (/^(no-reply|noreply|info|support|admin|service|notifications?|mailer-daemon)$/i.test(userPart) && domainPart) {
    const clean = domainPart.split('.')[0]; return clean.charAt(0).toUpperCase() + clean.slice(1);
  }
...
const formatSmartSubject = (subject: string, excerpt: string, fromAddress: string): string => {
  ...
  if (clean && clean !== '(Konu Yok)' && clean !== 'Konu Yok') return clean;
  ...
  if (/code|kod|verify|confirm/.test(combined)) return 'E-posta Doğrulama Kodu';
  if (/security|güvenlik/.test(combined)) return 'Güvenlik Bildirimi';
  return excerpt?.trim() ? excerpt.trim().slice(0, 45) + (excerpt.length > 45 ? '...' : '') : 'Gelen Mesaj';
};
...
export const getGuerrillaDomains = async (): Promise<string[]> => {
  ...
  try {
    ...
    const res = await fetch(`${GUERRILLA_API}?f=get_email_address&lang=en`, { signal: controller.signal }).catch(() => null); clearTimeout(timeoutId);
    if (res?.ok) { const data = await res.json().catch(() => null); const domain = String(data?.email_addr || '').split('@')[1]; if (domain) domains.add(domain); }
  } catch {}
  GUERRILLA_DOMAINS.forEach(d => domains.add(d)); return [...domains];
};
...
const createGuerrillaMailbox = async (emailUser?: string, domainName?: string): Promise<Mailbox> => {
  ...
  const data = await res.json(); const sid = typeof data?.sid_token === 'string' ? data.sid_token : '';
  if (!sid || typeof data?.email_addr !== 'string') throw new Error('Geçersiz Guerrilla Mail oturumu');
  let user = data.email_addr.split('@')[0]; const dom = domainName || data.email_addr.split('@')[1] || GUERRILLA_DOMAINS[0]; const targetUser = emailUser || user;
  const setRes = await safeFetch(`${GUERRILLA_API}?f=set_email_user&email_user=${encodeURIComponent(targetUser)}&lang=en&sid_token=${encodeURIComponent(sid)}`, undefined, 'guerrilla');
  if (setRes.ok) { const setData = await setRes.json().catch(() => null); if (typeof setData?.email_user === 'string') user = setData.email_user; }
  return { id: sid, address: `${user}@${dom}`, apiBase: 'guerrilla', token: sid, password: '', createdAt: Date.now() };
};
...
const getGuerrillaMessages = async (mailbox: Mailbox): Promise<EmailSummary[]> => {
  let sid = mailbox.token; const username = mailbox.address?.split('@')[0] || ''; if (!username) return [];
  try {
    if (!sid) { const r = await safeFetch(`${GUERRILLA_API}?f=get_email_address&lang=en`, undefined, 'guerrilla'); if (!r.ok) return []; const d = await r.json(); sid = d?.sid_token; if (!sid) return []; mailbox.token = sid; tokenRefreshListener?.(mailbox.id, sid); await safeFetch(`${GUERRILLA_API}?f=set_email_user&email_user=${encodeURIComponent(username)}&lang=en&sid_token=${encodeURIComponent(sid)}`, undefined, 'guerrilla'); }
    let res = await safeFetch(`${GUERRILLA_API}?f=get_email_list&offset=0&sid_token=${encodeURIComponent(sid)}`, undefined, 'guerrilla'); let data: any = res.ok ? await res.json().catch(() => null) : null;
    if (!res.ok || !data || data.error_codes || !Array.isArray(data.list)) { const renew = await safeFetch(`${GUERRILLA_API}?f=get_email_address&lang=en`, undefined, 'guerrilla'); if (renew.ok) { const rd = await renew.json().catch(() => null); if (rd?.sid_token) { sid = rd.sid_token; mailbox.token = sid; tokenRefreshListener?.(mailbox.id, sid); await safeFetch(`${GUERRILLA_API}?f=set_email_user&email_user=${encodeURIComponent(username)}&lang=en&sid_token=${encodeURIComponent(sid)}`, undefined, 'guerrilla'); res = await safeFetch(`${GUERRILLA_API}?f=get_email_list&offset=0&sid_token=${encodeURIComponent(sid)}`, undefined, 'guerrilla'); data = res.ok ? await res.json().catch(() => null) : null; } } }
    const seen = new Set<string>(); const list = (Array.isArray(data?.list) ? data.list : []).filter((msg: any) => { const id = String(msg?.mail_id || ''); if (!id || seen.has(id)) return false; seen.add(id); return !(String(msg?.mail_from || '').toLowerCase().includes('guerrillamail') && String(msg?.mail_subject || '').toLowerCase().includes('welcome')); });
    return list.map((msg: any) => { const fromAddr = String(msg.mail_from || 'unknown'); const subject = formatSmartSubject(decodeHTMLEntities(String(msg.mail_subject || '')), decodeHTMLEntities(String(msg.mail_excerpt || '')), fromAddr); const intro = decodeHTMLEntities(String(msg.mail_excerpt || '')); return { id: String(msg.mail_id), from: { address: fromAddr, name: formatSenderName(fromAddr) }, subject, intro: intro || 'Görüntülenecek önizleme yok', seen: msg.mail_read === 1, createdAt: parseGuerrillaDate(msg), aiCategory: determineCategory(subject, fromAddr, intro) }; });
  } catch { return []; }
};

const getGuerrillaMessageDetail = async (mailbox: Mailbox, messageId: string): Promise<EmailDetail | null> => {
  const sid = mailbox.token; if (!sid || !messageId) return null;
  try { const res = await safeFetch(`${GUERRILLA_API}?f=fetch_email&email_id=${encodeURIComponent(messageId)}&sid_token=${encodeURIComponent(sid)}`, undefined, 'guerrilla'); if (!res.ok) return null; const msg = await res.json(); if (!msg?.mail_id) return null; const subject = decodeHTMLEntities(String(msg.mail_subject || '')); const intro = decodeHTMLEntities(String(msg.mail_excerpt || '')); const fromAddr = typeof msg.mail_from === 'string' ? msg.mail_from : 'unknown'; return { id: String(msg.mail_id), from: { address: fromAddr, name: fromAddr.split('@')[0] || 'unknown' }, subject, intro, seen: true, createdAt: parseGuerrillaDate(msg), aiCategory: determineCategory(subject, fromAddr, intro), html: msg.mail_body ? [String(msg.mail_body)] : [], hasAttachments: false, attachments: [] }; } catch { return null; }
};
...
export const fetchDomains = async () => {
  if (cachedDomains?.domains.length) return cachedDomains;
  if (isFetchingDomains) { return new Promise<any>(resolve => { const started = Date.now(); const i = setInterval(() => { if (cachedDomains || Date.now() - started > 5000) { clearInterval(i); resolve(cachedDomains || { domains: GUERRILLA_DOMAINS, domainProviderMap: Object.fromEntries(GUERRILLA_DOMAINS.map(d => [d,'guerrilla'])), apiBase: 'guerrilla' }); } }, 100); }); }
  isFetchingDomains = true; try { const domains = await getGuerrillaDomains(); const map: Record<string,string> = {}; domains.forEach(d => { map[d] = 'guerrilla'; }); cachedDomains = { domains, domainProviderMap: map, apiBase: 'guerrilla' }; return cachedDomains; } finally { isFetchingDomains = false; }
};

export const generateMailbox = async (): Promise<Mailbox> => {
  const { domains, domainProviderMap } = await fetchDomains(); const list = domains.filter(d => d !== 'mephistomail.site'); const domainList = list.length ? list : GUERRILLA_DOMAINS; const domain = domainList[randomInt(domainList.length)]; const provider = domainProviderMap[domain] || 'guerrilla';
  const prefixes = ['matrix','vector','nexus','shadow','cyber','phantom','ninja','alpha','delta','vortex','hyper','pulse','signal','crypto']; const prefix = prefixes[randomInt(prefixes.length)]; const suffix = randomInt(900) + 100; const randomUser = `${prefix}.${Date.now().toString(36).slice(-5)}${suffix}`;
  if (isGuerrilla(provider)) return createGuerrillaMailbox(randomUser, domain);
  const cleanUser = randomUser.toLowerCase().trim().replace(/[^a-z0-9._-]/g,''); const address = `${cleanUser}@${domain}`; const password = generatePassword(); const apiBase = getApiBase(provider);
  const accRes = await safeFetch(`${apiBase}/accounts`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({address,password}) }, provider); if (!accRes.ok) throw new Error(`Hesap oluşturulamadı (HTTP ${accRes.status})`);
...
export const createCustomMailbox = async (username: string, domain: string, provider: string): Promise<Mailbox> => {
  if (!/^[a-zA-Z0-9._-]{1,64}$/.test(username) || !/^[a-zA-Z0-9.-]{1,253}$/.test(domain) || domain.startsWith('.') || domain.endsWith('.')) throw new Error('Geçersiz e-posta adresi');
  if (isGuerrilla(provider)) return createGuerrillaMailbox(username, domain);
  const apiBase = getApiBase(provider); const cleanUser = username.toLowerCase().trim().replace(/[^a-z0-9._-]/g,''); const address = `${cleanUser}@${domain.toLowerCase()}`; const password = generatePassword();
  const accRes = await safeFetch(`${apiBase}/accounts`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({address,password}) }, provider); if (accRes.status === 422) throw new Error('Bu kullanıcı adı zaten alınmış.'); if (!accRes.ok) throw new Error(`Hesap oluşturulamadı (HTTP ${accRes.status})`);
...
export const getMessageDetail = async (mailbox: Mailbox, messageId: string): Promise<EmailDetail | null> => {
  if (!mailbox.token || !messageId) return null; if (isGuerrilla(mailbox.apiBase)) return getGuerrillaMessageDetail(mailbox, messageId); const apiBase = getApiBase(mailbox.apiBase); const res = await safeFetch(`${apiBase}/messages/${encodeURIComponent(messageId)}`, { headers:{Authorization:`Bearer ${mailbox.token}`} }, mailbox.apiBase, mailbox.id); if (!res.ok) return null; const msg = await res.json(); const headerFields: Record<string,string> = { From:msg.from?.address || 'unknown', Subject:msg.subject || '', Date:msg.createdAt || '' }; if (msg.to?.length) headerFields.To = msg.to.map((t:any)=>t.address).join(', '); if (msg.cc?.length) headerFields.Cc = msg.cc.map((c:any)=>c.address).join(', '); if (msg.msgid) headerFields['Message-ID'] = msg.msgid; if (msg.size) headerFields.Size = `${msg.size} bytes`; return { id:msg.id, from:{address:msg.from?.address || 'unknown',name:msg.from?.name || msg.from?.address || 'unknown'}, subject:msg.subject || '', intro:msg.intro || '', seen:true, createdAt:msg.createdAt, aiCategory:determineCategory(msg.subject || '',msg.from?.address || '',msg.intro || ''), text:msg.text, html:msg.html ? (Array.isArray(msg.html) ? msg.html.map(String) : [String(msg.html)]) : [], hasAttachments:Boolean(msg.hasAttachments), attachments:(msg.attachments || []).map((a:any)=>({id:String(a.id || ''),filename:String(a.filename || 'attachment'),contentType:String(a.contentType || 'application/octet-stream'),size:Number(a.size || 0),downloadUrl:String(a.downloadUrl || '')})), headerFields };
};
export const deleteMessage = async (mailbox: Mailbox, messageId: string): Promise<boolean> => { if (!mailbox.token || !messageId) return false; if (isGuerrilla(mailbox.apiBase)) return deleteGuerrillaMessage(mailbox,messageId); try { const res = await safeFetch(`${getApiBase(mailbox.apiBase)}/messages/${encodeURIComponent(messageId)}`, {method:'DELETE',headers:{Authorization:`Bearer ${mailbox.token}`}}, mailbox.apiBase, mailbox.id); return res.ok || res.status === 204; } catch { return false; } };

export const subscribeToMailboxEvents = (mailbox: Mailbox, onEvent: () => void): (() => void) => {
  if (!mailbox?.token || isGuerrilla(mailbox.apiBase) || typeof EventSource === 'undefined') return () => {};
  try { const hubBase = mailbox.apiBase === 'mail_gw' ? 'https://mercure.mail.gw/.well-known/mercure' : 'https://mercure.mail.tm/.well-known/mercure'; const url = new URL(hubBase); url.searchParams.set('topic',`/accounts/${encodeURIComponent(mailbox.id)}`); const es = new EventSource(url.toString()); let errors=0; es.onmessage=()=>{errors=0;onEvent();}; es.onerror=()=>{if(++errors>=2) es.close();}; return ()=>es.close(); } catch { return ()=>{}; }
};

export interface SendEmailOptions { from:string; to:string; subject:string; text:string; mailboxId:string; sidToken?:string; }
export const sendEmail = async (options: SendEmailOptions): Promise<boolean> => {
  const sid = options.sidToken;
  if (!sid || !options.to || !options.from) return false;
  try { const formData = new URLSearchParams({f:'send_email',to:options.to,subject:options.subject.slice(0,998),body:`${options.text}\n\n---\nGönderen: ${options.from} (Mephisto Temp Mail)`,sid_token:sid}); const res = await safeFetch(GUERRILLA_API,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:formData.toString()},'guerrilla'); return res.ok; } catch { return false; }
};
