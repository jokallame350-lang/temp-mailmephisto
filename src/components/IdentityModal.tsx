...
const IdentityModal: React.FC<IdentityModalProps> = ({ isOpen, onClose, lang, activeAddress }) => {
    ...
    const copyBtn = (field: string, text: string) => (
        <button onClick={() => handleCopy(field, text)} className="p-1.5 hover:bg-white/10 rounded text-slate-400 transition-colors">
            ...
            <div className="bg-[#0a0a0c] border border-white/10 rounded-xl shadow-2xl w-full max-w-sm overflow-hidden text-white relative z-10 animate-fade-in-up">
                <div className="flex items-center justify-between p-4 border-b border-white/5 bg-[#111]">
                    <h3 className="text-sm font-bold uppercase flex items-center gap-2 text-indigo-400">
                        ...
                        {/* Fallback translation strings to not break existing instances */}
                        {t.identityTitle || 'Burner Identity'}
                    </h3>
                ...
                <div className="p-5 space-y-4">
                    <p className="text-xs text-slate-400 leading-relaxed mb-2 text-center">
                        {lang === 'tr' ? 'Bu e-posta adresi için özel olarak oluşturulmuş, hep aynı kalan sahte bir dijital kimlik. Anonim kalmak istediğiniz formlarda güvenle kullanabilirsiniz.' : 'This is a consistent, fake digital identity generated specifically for this email address. Use it for anonymous registrations.'}
                    </p>
                    ...
                    <div className="space-y-3">
                        <div className="flex flex-col gap-1 p-3 bg-black/40 border border-white/5 rounded-lg group hover:border-indigo-500/50 transition-colors">
                            <span className="text-[10px] sm:text-xs text-slate-500 uppercase tracking-widest font-bold flex items-center gap-1.5"><UserCheck className="w-3 h-3" /> {t.name || 'Name'}</span>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="flex flex-col gap-1 p-3 bg-black/40 border border-white/5 rounded-lg group hover:border-indigo-500/50 transition-colors">
                                <span className="text-[10px] sm:text-xs text-slate-500 uppercase tracking-widest font-bold flex items-center gap-1.5"><Calendar className="w-3 h-3" /> {t.birthday || 'Birthday'}</span>
                            ...
                            <div className="flex flex-col gap-1 p-3 bg-black/40 border border-white/5 rounded-lg group hover:border-indigo-500/50 transition-colors">
                                <span className="text-[10px] sm:text-xs text-slate-500 uppercase tracking-widest font-bold flex items-center gap-1.5"><Phone className="w-3 h-3" /> {t.phone || 'Phone'}</span>
                        ...
                        <div className="flex flex-col gap-1 p-3 bg-black/40 border border-white/5 rounded-lg group hover:border-indigo-500/50 transition-colors">
                            <span className="text-[10px] sm:text-xs text-slate-500 uppercase tracking-widest font-bold flex items-center gap-1.5"><Briefcase className="w-3 h-3" /> {t.job || 'Occupation'}</span>
                        ...
                        <div className="flex flex-col gap-1 p-3 bg-black/40 border border-white/5 rounded-lg group hover:border-indigo-500/50 transition-colors">
                            <span className="text-[10px] sm:text-xs text-slate-500 uppercase tracking-widest font-bold flex items-center gap-1.5"><MapPin className="w-3 h-3" /> {t.address || 'Address'}</span>
