...
function getCredits(): number {
  try {
    ...
    if (saved === null) return FREE_CREDITS;
...
const RewardedAdModal: React.FC<RewardedAdModalProps> = ({ isOpen, onClose, onCreditsEarned, lang }) => {
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className="w-full max-w-md bg-[#0a0a0f] border border-white/10 rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-150"
      >
        {/* Header */}
        <div className="relative p-6 pb-4 text-center">
          <button
            ...
            onClick={onClose}
            aria-label={t.close}
            className="absolute top-4 right-4 p-1.5 text-slate-500 hover:text-white rounded-lg transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-500"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          {adState === 'completed' ? (
            <>
              ...
              <h2 id="rewarded-modal-title" className="text-xl font-black text-white">{t.completed}</h2>
            ...
            <>
              ...
              <h2 id="rewarded-modal-title" className="text-xl font-black text-white">{t.title}</h2>
          ...
          {adState === 'watching' ? (
            <div className="text-center py-8">
              <div className="w-full h-40 bg-white/[0.02] border border-white/5 rounded-xl flex flex-col items-center justify-center mb-4">
                <Film className="w-8 h-8 text-slate-500 mb-2 animate-pulse" aria-hidden="true" />
              ...
              <div className="flex items-center justify-center gap-2 text-amber-400">
                <Timer className="w-4 h-4 animate-spin" aria-hidden="true" />
            ...
            <button
              ...
              className="w-full py-3.5 bg-gradient-to-r from-green-600 to-emerald-600 text-white font-black rounded-xl hover:opacity-90 transition-all active:scale-[0.98] flex items-center justify-center gap-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-green-400 shadow-lg shadow-green-600/20"
            >
            ...
            <button
              ...
              className="w-full py-3.5 bg-gradient-to-r from-red-600 to-orange-600 text-white font-black rounded-xl hover:opacity-90 transition-all active:scale-[0.98] flex items-center justify-center gap-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-400 shadow-lg shadow-orange-600/20"
            >
