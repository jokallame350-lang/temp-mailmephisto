    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-[#0a0a0c] border border-white/10 rounded-xl shadow-2xl w-full max-w-md overflow-hidden text-white">
        ...
        <div className="p-6 space-y-6">
          <div className="relative flex items-center bg-black border border-white/10 rounded-lg p-4 justify-between">
            <span className="font-mono text-lg text-emerald-400 break-all mr-2">{password}</span>
            <div className="flex items-center gap-2">
              <button onClick={generatePassword} className="p-2 hover:bg-white/10 rounded-md text-slate-400"><RefreshCw className="w-4 h-4" /></button>
              <button onClick={handleCopy} className="p-2 bg-white/10 rounded-md text-white">{copied ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}</button>
            </div>
          </div>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <input type="range" min="8" max="64" value={length} onChange={(e) => setLength(Number(e.target.value))} className="w-full h-2 bg-white/10 rounded-lg appearance-none cursor-pointer accent-emerald-500" />
            </div>
