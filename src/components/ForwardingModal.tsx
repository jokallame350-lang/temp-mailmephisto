...
const ForwardingModal: React.FC<ForwardingModalProps> = ({ isOpen, onClose, lang, activeAddress }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="bg-[#0a0a0c] border border-white/10 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden text-white relative z-10 animate-fade-in-up">
                {/* Modal Header */}
                ...
                <div className="p-5 sm:p-6 space-y-5">
                    {/* Private Beta / Infrastructure Setup Notice */}
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex flex-col gap-1 p-3 bg-black/40 border border-white/5 rounded-xl focus-within:border-rose-500/50 transition-colors">
                            ...
                            <input
                                ...
                                className="w-full bg-transparent border-none text-sm font-mono text-white focus:outline-none placeholder-slate-600"
                            />
                        </div>
                    </div>

                    {/* Saved State / Queue Information Status Card */}
                    {isSaved && savedPref && (
                        <div className="p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-xs text-emerald-300 space-y-2 animate-fade-in">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-2 pt-1">
                        <button
                            ...
                            onClick={handleSave}
                            className={`w-full py-3 rounded-xl flex items-center justify-center gap-2 text-xs font-bold transition-all shadow-lg ${
                                !isValidEmail
                        ...
                        {isSaved && (
                            <button
                                onClick={handleClear}
                                className="w-full sm:w-auto px-4 py-3 rounded-xl bg-white/5 hover:bg-red-500/20 text-slate-400 hover:text-red-400 border border-white/10 text-xs font-bold transition-colors flex items-center justify-center gap-1.5 shrink-0"
