...
const AddressBar: React.FC<AddressBarProps> = ({ mailbox, isLoading, isRefreshing, onRefresh, onChange, onDelete: _onDelete, progress, lang, onChangeDomain, onCreateCustom, onIdentity, onForwarding, onCopyMagicUrl }) => {
  ...
  return <div className="w-full flex flex-col items-center gap-4 sm:gap-6 max-w-2xl mx-auto px-3 sm:px-4">
    <div role="button" tabIndex={0} onClick={handleCopy} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleCopy(); } }} className="w-full relative group cursor-pointer active:scale-[0.99] transition-transform duration-200 focus:outline-none focus:ring-2 focus:ring-orange-500/50 rounded-2xl" title={t.tipCopy}>
      <div className="relative bg-[#0f1115] border border-white/10 rounded-xl sm:rounded-2xl p-1 shadow-xl flex items-center overflow-visible h-14 sm:h-16">
        ...
        <div className="pr-2 sm:pr-4 pl-1 sm:pl-2 flex items-center gap-1.5 sm:gap-2.5 flex-shrink-0"><CountdownBadge createdAt={mailbox?.createdAt} lang={lang} /><button type="button" onClick={e => { e.stopPropagation(); handleCopyMagicUrl(); }} className="p-1.5 rounded-lg bg-white/5 hover:bg-orange-500/20 text-slate-400 hover:text-orange-400" title={lang === 'tr' ? 'Mailbox bağlantısını kopyala' : 'Copy mailbox link'}>{copiedLink ? <Check className="w-4 h-4 text-green-500" /> : <Share2 className="w-4 h-4 text-orange-400" />}</button><button type="button" onClick={e => { e.stopPropagation(); handleCopy(); }} className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest px-2 sm:px-2.5 py-1 rounded-lg bg-orange-500/20 text-orange-400 hover:bg-orange-500/30 border border-orange-500/30 transition-all flex items-center gap-1">{copied ? <Check className="w-3 h-3 text-green-400" /> : <Copy className="w-3 h-3" />}<span>{copied ? t.copied : t.copy}</span></button></div>
      </div>
    </div>
    <div className="flex flex-col gap-3 w-full items-center"><div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 w-full">
      <button onClick={onRefresh} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[#111] text-slate-200 border border-white/10 hover:border-orange-500/40 rounded-full font-bold text-xs min-h-[44px] transition-all active:scale-95"><RefreshCw className={`w-4 h-4 text-orange-400 ${isRefreshing ? 'animate-spin' : ''}`} /><span>{t.refresh}</span></button>
      <button onClick={onChange} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-orange-500/10 text-orange-400 hover:bg-orange-500/20 border border-orange-500/30 rounded-full font-bold text-xs min-h-[44px] transition-all active:scale-95"><PlusCircle className="w-4 h-4 text-orange-400" /><span>{lang === 'tr' ? 'Yeni Mailbox' : 'New Mailbox'}</span></button>
      {onCreateCustom && <button onClick={onCreateCustom} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[#111] text-amber-300 hover:bg-amber-500/10 border border-amber-500/30 rounded-full font-bold text-xs min-h-[44px] transition-all active:scale-95"><Sparkles className="w-4 h-4 text-amber-400" /><span>{lang === 'tr' ? 'Özel Adres' : 'Custom'}</span></button>}
      <button onClick={handleCopyMagicUrl} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[#111] text-slate-300 hover:text-white border border-white/10 hover:border-orange-500/30 rounded-full font-bold text-xs min-h-[44px] transition-all active:scale-95">{copiedLink ? <Check className="w-4 h-4 text-green-400" /> : <Link className="w-4 h-4 text-orange-400" />}<span>Magic URL</span></button>
      <button onClick={handleTestOtpTrigger} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-amber-500/10 text-amber-300 hover:bg-amber-500/20 border border-amber-500/30 rounded-xl font-bold text-xs min-h-[44px] transition-all active:scale-95" title={lang === 'tr' ? 'Canlı test OTP mesajı gönder' : 'Send live test OTP message'}><Zap className="w-4 h-4 text-amber-400" /><span>Test OTP</span></button>
      {onIdentity && <button onClick={onIdentity} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white/[0.03] text-slate-400 hover:text-white border border-white/10 rounded-xl font-bold text-xs min-h-[44px] transition-all"><UserCheck className="w-4 h-4" /><span>{lang === 'tr' ? 'Kimlik' : 'Identity'}</span></button>}
      {onForwarding && <button onClick={onForwarding} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-rose-500/10 text-rose-400 hover:bg-rose-500/20 border border-rose-500/20 rounded-xl font-bold text-xs min-h-[44px] transition-all" title={lang === 'tr' ? 'Hayalet Yönlendirme (Beta)' : 'Ghost Forwarding (Beta)'}><Ghost className="w-4 h-4 text-rose-500" /><span>{lang === 'tr' ? 'Yönlendirme' : 'Forward'}</span></button>}
      <button onClick={exportMailboxTxt} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 border border-blue-500/20 rounded-xl font-bold text-xs min-h-[44px] transition-all"><FileText className="w-4 h-4" /><span>TXT</span></button>
      <button onClick={exportMailboxCsv} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 border border-emerald-500/20 rounded-xl font-bold text-xs min-h-[44px] transition-all"><FileSpreadsheet className="w-4 h-4" /><span>CSV</span></button>
    </div></div>
  </div>;
};
